...
# 创建数据框架
data <- data.frame(tree = query_sample2$UTC1500, sentiment = query_sample2$sentiment, period=query_sample2$period)
## 获取样本对应的子集数据框
...
ggplot(data_meanUTC_before, mapping=aes(x= tree, y=sentiment))+
  ...
  annotate("text",  x=0.35,   y=24.2, size=5,color="black",hjust=0,label = "italic(P[before]) < 0.001", parse = TRUE, parse = TRUE)+
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'tree_sentiment_meanUTC_before', '.pdf', sep="")  # 字符串连接
...
ggplot(data_meanUTC_during, mapping=aes(x= tree, y=sentiment))+
  ...
  stat_poly_eq(
    ...
    aes(label = paste(..eq.label.., sep = "~~~")),
    parse = TRUE,
    label.x = 0.8, label.y = 0.75, #设置位置
    size=5, color = "black"  # 设置字体大小和颜色
  )+
  annotate("text",  x=0.35, y=35.6, size=5,color="black",hjust=0,label = "italic(P[during]) < 0.001", parse = TRUE, parse = TRUE)+
  theme_classic()+theme0+
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'tree_sentiment_meanUTC_during', '.pdf', sep="")  # 字符串连接
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'tree_sentiment_meanUTC_before_meanUTC_during', '.pdf', sep="")  # 字符串连接
...
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_effects_before', '.csv', sep="")  # 字符串连接
effects_UTC1500_UTC_meanUTC_before = read.csv(input_file_name_meanUTC_before, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                              strip.white = FALSE)

input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_effects_during', '.csv', sep="")  # 字符串连接
effects_UTC1500_UTC_meanUTC_during = read.csv(input_file_name_meanUTC_during, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                              strip.white = FALSE)
...
effects_UTC1500_UTC_meanUTC_before$period <- "Before Covid-19"
effects_UTC1500_UTC_meanUTC_during$period <- "During Covid-19"

combined_data1 <- rbind(effects_UTC1500_UTC_meanUTC_before,
                        effects_UTC1500_UTC_meanUTC_during)
...
ggplot(data = combined_data1, aes(x = UTC, y = mean_value, color = period, fill = period)) +
  ...
  annotate("text",  x=0.2,  y= -0.3, size=6, color="black",hjust=0,label = "italic(P[before]) < 0.001", parse = TRUE, parse = TRUE)+
  annotate("text",  x=0.2,  y = -0.8, size=6, color = "black", hjust = 0, label = "italic(P[during]) < 0.001", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0, 0.6), breaks = seq(0, 0.6, by =0.15), 
                     labels = scales::number_format(accuracy = 0.01)) +
  scale_y_continuous(name = "Effect on negative sentiment \n  values (95% CI)", 
                     limits = c(-3.8, 1),
                     breaks = seq(-3.8, 1, by = 2), 
                     labels = scales::number_format(accuracy = 0.1)) +
...
# 绘制直方图，边框颜色与填充颜色不同
ggplot(data = query_sample2, aes(x = UTC1500, fill = period)) +
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_effects', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_UTC1500_before', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_UTC1500_meanUTC_before = read.csv(input_file_name_meanUTC_before, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                      strip.white = FALSE)


##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_UTC1500_during', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_UTC1500_meanUTC_during = read.csv(input_file_name_meanUTC_during, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                      strip.white = FALSE)
...
##################################
# 合并数据集，增加一个标记变量 "period" 表示疫情前后
marginaleffects_UTC_UTC1500_meanUTC_before$period <- "Before Covid-19"
marginaleffects_UTC_UTC1500_meanUTC_during$period <- "During Covid-19"

combined_data1 <- rbind(marginaleffects_UTC_UTC1500_meanUTC_before,
                        marginaleffects_UTC_UTC1500_meanUTC_during)




### 绘图
ggplot(data = combined_data1, aes(x = UTC1500, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  annotate("text", x = 0.1, y = 18, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 0.1, y = 14, size = 6, color = "black", hjust = 0, label = "italic(P[during]) < 0.001", parse = TRUE) +
  ...
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-22.0, 20), breaks = seq(-22.0, 20, by = 10.0),
                     labels = scales::number_format(accuracy = 0.1)) +
...
ggplot(data = query_sample2, aes(x = UTC1500, fill = period)) +
  ...
  theme(plot.margin = margin(t = -0.35, b = 0, l = 0.2, r = 0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),
        axis.text.x = element_text(color = "black", size = 15),
        legend.position = "none")->pUTC15002
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_UTC1500', '.pdf', sep="")  # 字符串连接
...
a2 <-  cowplot::plot_grid(pUTC1500,nrow = 1, ncol = 1, labels = c("b"),
                          label_size = 15, scale=1,label_x = 0.1, label_y = 1.0, rel_heights = c(1.0, 0.1), rel_widths = c(1, 1))
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_effect_Marginaleffect', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_NOUTC1500_meanUTC_before', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_NOUTC1500_meanUTC_before = read.csv(input_file_name_meanUTC_before, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                        strip.white = FALSE)


##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_NOUTC1500_meanUTC_during', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_NOUTC1500_meanUTC_during = read.csv(input_file_name_meanUTC_during, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                        strip.white = FALSE)
...
##################################
# 合并数据集，增加一个标记变量 "period" 表示疫情前后
marginaleffects_UTC_NOUTC1500_meanUTC_before$period <- "Before Covid-19"
marginaleffects_UTC_NOUTC1500_meanUTC_during$period <- "During Covid-19"

combined_data1 <- rbind(marginaleffects_UTC_NOUTC1500_meanUTC_before,
                        marginaleffects_UTC_NOUTC1500_meanUTC_during)

### 绘图
ggplot(data = combined_data1, aes(x = NOUTC1500, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线 
  annotate("text", x = 0.1, y = -1.0, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 0.1, y = -3.5, size = 6, color = "black", hjust = 0, label = "italic(P[during]) > 0.1", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits = c(0, 0.6), breaks = seq(0, 0.6, by = 0.15),
                     labels = scales::number_format(accuracy = 0.01)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-20.0, 0), breaks = seq(-20.0, 0, by = 5.0),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
# 绘制直方图，边框颜色与填充颜色不同
ggplot(data = query_sample2, aes(x = NOUTC1500, fill = period)) +
  geom_histogram(aes(y = ..density.., color = period), 
                 position = "identity", binwidth = 0.001, size = 0.8, alpha = 0.4, 
                 color = NA) +  # 设置边框颜色为透明
  ...
  theme(plot.margin = margin(t = -0.35, b = 0, l = 0.2, r = 0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),
        axis.text.x = element_text(color = "black", size = 15),
        legend.position = "none")->pNOUTC15002
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_NOUTC1500', '.pdf', sep="")  # 字符串连接
...
bb <-  cowplot::plot_grid(pUTC1500,nrow = 1, ncol = 1, labels = c("b"),
                          label_size = 18, scale=1,label_x = 0.02, label_y = 1.0, rel_widths = c(1, 1))
cc <-  cowplot::plot_grid(pNOUTC1500,nrow = 1, ncol = 1, labels = c("c"),
                          label_size = 18, scale=1,label_x = 0.02, label_y = 1.0, rel_widths = c(1, 1))
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC1500_effect_Marginal_effect', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Precipitation_meanUTC_before', '.csv', sep="")  # 字符串连接
...
##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Precipitation_meanUTC_during', '.csv', sep="")  # 字符串连接
...
ggplot(data = combined_data1, aes(x = Precipitation, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  annotate("text", x = 5, y = -4.0, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 5, y = -5.5, size = 6, color = "black", hjust = 0, label = "italic(P[during])< 0.001", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits = c(0, 45), breaks = seq(0, 45, by =11.0),
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-16.0, -0), breaks = seq(-16.0, -0, by = 4.0),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Precipitation', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_humidity_meanUTC_before', '.csv', sep="")  # 字符串连接
...
##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_humidity_meanUTC_during', '.csv', sep="")  # 字符串连接
...
ggplot(data = combined_data1, aes(x = humidity, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  annotate("text", x = 30, y = -2.0, size = 6, color = "black", hjust = 0, label = "italic(P[before]) > 0.1", parse = TRUE) +
  annotate("text", x = 30, y = -4.0, size = 6, color = "black", hjust = 0, label = "italic(P[during]) < 0.001", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits = c(15, 100), breaks = seq(15, 100, by =21),   
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-18.0, 0), breaks = seq(-18.0, 0, by = 4.0),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_humidity', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Surface_pressure_meanUTC_before', '.csv', sep="")  # 字符串连接
...
##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Surface_pressure_meanUTC_during', '.csv', sep="")  # 字符串连接
...
ggplot(data = combined_data1, aes(x = Surface_pressure, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  annotate("text", x = 91.5, y = -2.5, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 91.5, y = -4.5, size = 6, color = "black", hjust = 0, label = "italic(P[during]) < 0.001", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits =  c(90, 102), breaks = seq(90, 102, by =3), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-18.0, 0), breaks = seq(-18.0, 0, by = 3),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
ggplot(data = query_sample2, aes(x = Surface_pressure, fill = period)) +
  geom_histogram(aes(y = ..density.., color = period), 
                 position = "identity", binwidth = 0.001, size = 0.8, alpha = 0.4, 
                 color = NA) +  # 设置边框颜色为透明
  scale_x_continuous(expression(Surface~Pressure~(Pa~"×"~1500)),
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Surface_pressure', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Skintemperature_meanUTC_before', '.csv', sep="")  # 字符串连接
...
##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Skintemperature_meanUTC_during', '.csv', sep="")  # 字符串连接
...
ggplot(data = combined_data1, aes(x = Skintemperature, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  annotate("text", x = 10, y = -2.0, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 10, y = -4.0, size = 6, color = "black", hjust = 0, label = "italic(P[during]) < 0.001", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits = c(5, 45), breaks = seq(5, 45, by =10),  
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-20.0, 0), breaks = seq(-20.0, 0, by = 4.0),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_Skintemperature', '.pdf', sep="")  # 字符串连接
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC1500_climate_Marginal_effect', '.pdf', sep="")  # 字符串连接
...
#########################Marginal effect population1500 ################################

##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_population1500_meanUTC_before', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_population1500_meanUTC_before = read.csv(input_file_name_meanUTC_before, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                             strip.white = FALSE)


##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_population1500_meanUTC_during', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_population1500_meanUTC_during = read.csv(input_file_name_meanUTC_during, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                             strip.white = FALSE)
...
##################################
# 合并数据集，增加一个标记变量 "period" 表示疫情前后
marginaleffects_UTC_population1500_meanUTC_before$period <- "Before Covid-19"
marginaleffects_UTC_population1500_meanUTC_during$period <- "During Covid-19"

combined_data1 <- rbind(marginaleffects_UTC_population1500_meanUTC_before,
                        marginaleffects_UTC_population1500_meanUTC_during)

### 绘图
ggplot(data = combined_data1, aes(x = population1500, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  annotate("text", x = 0.53, y = -5.5, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 0.53, y = -7.5, size = 6, color = "black", hjust = 0, label = "italic(P[during]) > 0.1", parse = TRUE) + 
  
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits = c(0, 6), breaks = seq(0, 6, by =1.5), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-22.0, 0), breaks = seq(-22.0, 0, by = 5),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
# 绘制直方图，边框颜色与填充颜色不同
ggplot(data = query_sample2, aes(x = population1500, fill = period)) +
  geom_histogram(aes(y = ..density.., color = period), 
                 position = "identity", binwidth = 0.001, size = 0.8, alpha = 0.4, 
                 color = NA) +  # 设置边框颜色为透明
  scale_x_continuous(name =  expression(Population~density~(~"×"~1500)), 
  ...
  theme(plot.margin = margin(t = -0.35, b = 0, l = 0.2, r = 0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),
        axis.text.x = element_text(color = "black", size = 15),
        legend.position = "none")->ppopulation15002
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_population1500', '.pdf', sep="")  # 字符串连接
...
#########################Marginal effect nightlight1500 ################################

##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_nightlight1500_meanUTC_before', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_nightlight1500_meanUTC_before = read.csv(input_file_name_meanUTC_before, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                             strip.white = FALSE)


##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_nightlight1500_meanUTC_during', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_nightlight1500_meanUTC_during = read.csv(input_file_name_meanUTC_during, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                             strip.white = FALSE)
...
##################################
# 合并数据集，增加一个标记变量 "period" 表示疫情前后
marginaleffects_UTC_nightlight1500_meanUTC_before$period <- "Before Covid-19"
marginaleffects_UTC_nightlight1500_meanUTC_during$period <- "During Covid-19"

combined_data1 <- rbind(marginaleffects_UTC_nightlight1500_meanUTC_before,
                        marginaleffects_UTC_nightlight1500_meanUTC_during)

### 绘图
ggplot(data = combined_data1, aes(x = nightlight1500, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  
  annotate("text", x = 30, y = -2.0, size = 6, color = "black", hjust = 0, label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 30, y = -4.0, size = 6, color = "black", hjust = 0, label = "italic(P[during]) < 0.001", parse = TRUE) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加红色虚线
  scale_x_continuous(name = " ", limits = c(0, 120), breaks = seq(0, 120, by =30),  
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-22.0, 0), breaks = seq(-22.0, 0, by = 5.0),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
# 绘制直方图，边框颜色与填充颜色不同
ggplot(data = query_sample2, aes(x = nightlight1500, fill = period)) +
  geom_histogram(aes(y = ..density.., color = period), 
  ...
  theme(plot.margin = margin(t = -0.35, b = 0, l = 0.2, r = 0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),
        axis.text.x = element_text(color = "black", size = 15),
        legend.position = "none")->pnightlight15002
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_nightlight1500', '.pdf', sep="")  # 字符串连接
...
#########################Marginal effect settlement1500 ################################

##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_settlement1500_meanUTC_before', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_settlement1500_meanUTC_before = read.csv(input_file_name_meanUTC_before, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                             strip.white = FALSE)


##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_settlement1500_meanUTC_during', '.csv', sep="")  # 字符串连接

marginaleffects_UTC_settlement1500_meanUTC_during = read.csv(input_file_name_meanUTC_during, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                             strip.white = FALSE)
...
##################################
# 合并数据集，增加一个标记变量 "period" 表示疫情前后
marginaleffects_UTC_settlement1500_meanUTC_before$period <- "Before Covid-19"
marginaleffects_UTC_settlement1500_meanUTC_during$period <- "During Covid-19"

combined_data1 <- rbind(marginaleffects_UTC_settlement1500_meanUTC_before,
                        marginaleffects_UTC_settlement1500_meanUTC_during)

### 绘图
ggplot(data = combined_data1, aes(x = settlement1500, y = mean_value, color = period, fill = period)) +
  geom_ribbon(aes(ymin = Low_value, ymax = UP_value), alpha = 0.25, color = NA) +  # 去掉边界线
  geom_line(size = 0.9) +  # 平均值线
  ...
  scale_x_continuous(name = " ", limits = c(0, 100), breaks = seq(0, 100, by =25), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-25.0, 0), breaks = seq(-25.0, 0, by = 6),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
...
# 绘制直方图，边框颜色与填充颜色不同
ggplot(data = query_sample2, aes(x = settlement1500, fill = period)) +
  geom_histogram(aes(y = ..density.., color = period), 
  ...
  theme(plot.margin = margin(t = -0.35, b = 0, l = 0.2, r = 0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),
        axis.text.x = element_text(color = "black", size = 15),
        legend.position = "none")->psettlement15002
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_settlement1500', '.pdf', sep="")  # 字符串连接
...
##疫情前数据集
input_file_name_meanUTC_before= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_GDP_meanUTC_before', '.csv', sep="")  # 字符串连接
...
##疫情后数据集
input_file_name_meanUTC_during= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_GDP_meanUTC_during', '.csv', sep="")  # 字符串连接
...
ggplot(data = combined_data1, aes(x = GDP, y = mean_value, color = period, fill = period)) +
  ...
  annotate("text", x = 5, y = -2.5, size = 6, color = "black", hjust = 0,
           label = "italic(P[before]) < 0.001", parse = TRUE) +
  annotate("text", x = 5, y = -4.5, size = 6, color = "black", hjust = 0,
           label = "italic(P[during]) < 0.001", parse = TRUE) + 
  ...
  scale_x_continuous(name = " ", limits = c(3, 23), breaks = seq(3, 23, by =5), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)",
                     limits = c(-20, 0), breaks = seq(-20, 0, by = 5),
                     labels = scales::number_format(accuracy = 0.1)) +
  #scale_color_manual(values = c("Before Covid-19" = "#FFB5C5", "During Covid-19" = "#48D1CC")) + # 填充颜色  # 自定义颜色
  #scale_fill_manual(values = c("Before Covid-19" = "#1F78B466", "During Covid-19" = "#33A02C66")) +
  labs(title = "", y = "Marginal effect on negative \n sentiment values (95% CI)", x = "Skin Temperature") +
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC_GDP', '.pdf', sep="")  # 字符串连接
...
kk <-  cowplot::plot_grid(psettlement1500,nrow = 1, ncol = 1, labels = c("c"),
                          label_size = 18, scale=1,label_x = 0.02, label_y = 1.0, rel_widths = c(1, 1))
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes_covid19', '/secondary_data_utc1500/',  'UTC1500_envrionments_Marginal_effect', '.pdf', sep="")  # 字符串连接

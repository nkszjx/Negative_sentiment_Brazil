...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_effects', '.csv', sep="")  # 字符串连接

effects_UTC500_UTC = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                              strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = effects_UTC500_UTC)+
  ...
  annotate("text",x=0.4, y=-0.5, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE, parse = TRUE)+
...
ggplot(data = query_sample2) +
  geom_histogram(aes(x = UTC500, y = ..density..), binwidth = 0.002, fill = "#002C5B66", color = "#002C5B66", alpha = 0.5) +
  #geom_density(aes(x=UTC500),fill="#002C5B66", color="white",show.legend = F, adjust = 2.5)+
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_effects', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_UTC500', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_UTC500 = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                         strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_UTC500)+
  geom_ribbon(aes(x=UTC500, y=mean_value, ymin=Low_value, ymax=UP_value),fill="#ED0021",alpha=0.3)+
  geom_line(aes(x=UTC500, y=mean_value),colour="black",size=0.70)+
  annotate("text",x=0.15, y=-2.0, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0, 0.8), breaks = seq(0.0, 0.8, by =0.2), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-10.0, 0.0),
                     breaks = seq(-10.0, 0.0, by = 2.0), 
                     labels = scales::number_format(accuracy = 0.1)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
...
#######################
ggplot(data = query_sample2)+
  geom_histogram(aes(x = UTC500, y = ..density..), binwidth = 0.0020, fill = "#002c5B66", color = "#002c5B66", alpha = 0.5) +
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_UTC500', '.pdf', sep="")  # 字符串连接
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_effect_UTC500_Marginal_effect', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_NOUTC500', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_NOUTC500 = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                           strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_NOUTC500)+
  geom_ribbon(aes(x=NOUTC500, y=mean_value, ymin=Low_value, ymax=UP_value),fill="#ED0021",alpha=0.3)+
  geom_line(aes(x=NOUTC500, y=mean_value),colour="black",size=0.7)+
  annotate("text",x=0.55, y=-2.7, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0, 0.8), breaks = seq(0, 0.8, by =0.2), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-12, 0),
...
#######################
ggplot(data = query_sample2)+
  geom_histogram(aes(x = NOUTC500, y = ..density..), binwidth = 0.0015, fill = "#002C5B66", color = "#002c5B66", alpha = 0.5) +
  #geom_density(aes(x=NOUTC500),fill="#002C5B66", color="white",show.legend = F, adjust = 2.5)+
  ...
  scale_y_continuous(name = "  ", limits = c(0, 11),
                     breaks = seq(0, 11, by = 3), 
                     labels = scales::number_format(accuracy = 0.1)) +
  #labs(x=expression("NOUTC500"),y="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.35,b=0,l=0.2,r=0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
        axis.text.x = element_text(color = "black", size = 15)  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
  )->pNOUTC5002 # 可视范围限制在 0 到 0.5 的范围内
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_NOUTC500', '.pdf', sep="")  # 字符串连接

ggsave(save_image_path, plot = pNOUTC500, width = 150, height = 130, units = "mm",dpi=200)


################################################合并图形###############################################################
dd <-  cowplot::plot_grid(pUTC500,nrow = 1, ncol = 1, labels = c("b"),
                          label_size = 18, scale=1,label_x = 0.02, label_y = 1.0, rel_widths = c(1, 1))



cc <-  cowplot::plot_grid(pNOUTC500,nrow = 1, ncol = 1, labels = c("a"),
                          label_size = 18, scale=1,label_x = 0.02, label_y = 1.0, rel_widths = c(1, 1))
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC500_NOUTC500_Marginal_effect', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Precipitation', '.csv', sep="")  # 字符串连接

marginaleffects_UTC500_Precipitation = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_Precipitation)+
  ...
  annotate("text",x=11, y= -1.4, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Precipitation', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_humidity', '.csv', sep="")  # 字符串连接



marginaleffects_UTC500_humidity= read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                          strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_humidity)+
  ...
  annotate("text",x=36, y=-1.5, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(15, 100), breaks = seq(15, 100, by =21), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-6.0, 0.2),
                     breaks = seq(-6.0, 0.2, by = 1.5), 
                     labels = scales::number_format(accuracy = 0.1)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_humidity', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Wind', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_Wind = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                       strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_Wind)+
  ...
  annotate("text",x=1.5, y=-1.5, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Wind', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_cloudcover', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_cloudcover = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                             strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_cloudcover)+
  ...
  annotate("text",x=20, y=-1.5, size=5,color="grey20",hjust=0,label = "italic(P) = 00.8152", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_cloudcover', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Surface_pressure', '.csv', sep="")  # 字符串连接

marginaleffects_UTC500_Surface_pressure = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                   strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_Surface_pressure)+
  ...
  annotate("text",x=92, y=-2.0, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(88, 104), breaks = seq(88, 104, by =4), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-8.0, 0.2),
                     breaks = seq(-8.0, 0.2, by = 2.0), 
                     labels = scales::number_format(accuracy = 0.1)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Surface_pressure', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Skintemperature', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_Skintemperature = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                  strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_Skintemperature)+
  ...
  annotate("text",x=15, y=-1.8, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(5, 45), breaks = seq(5, 45, by =10), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-7.2, 0.2),
                     breaks = seq(-7.2, 0.2, by = 1.8), 
                     labels = scales::number_format(accuracy = 0.1)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_Skintemperature', '.pdf', sep="")  # 字符串连接
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC500_climate_Marginal_effect', '.pdf', sep="")  # 字符串连接

ggsave(save_image_path, plot = p5, width = 260, height = 240, units = "mm",dpi=200)
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_population500', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_population500 = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_population500)+
  geom_ribbon(aes(x=population500, y=mean_value, ymin=Low_value, ymax=UP_value),fill="#ED0021",alpha=0.3)+
  geom_line(aes(x=population500, y=mean_value),colour="black",size=0.7)+
  annotate("text",x= 2.5, y= -1.2, size=5,color="grey20",hjust=0,label = "italic(P) < 0.005", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0, 10), breaks = seq(0, 10, by =2.5), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-4.8, 0),
                     breaks = seq(-4.8, 0, by = 1.2), 
                     labels = scales::number_format(accuracy = 0.1)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
...
#######################
ggplot(data = query_sample2)+
  geom_histogram(aes(x = population500, y = ..density..), binwidth = 0.015, fill = "#002C5B66", color = "#002c5B66", alpha = 0.5) +
  #geom_density(aes(x=population500),fill="#002C5B66", color="white",show.legend = F, adjust = 1.5)+
  ...
  scale_y_continuous(name = "  ", limits = c(0, 0.65),
                     ...
                     labels = scales::number_format(accuracy = 0.2)) +
  #labs(x=expression("population500"),y="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.35,b=0,l=0.2,r=0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
        axis.text.x = element_text(color = "black", size = 15)  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
  )->ppopulation5002 # 可视范围限制在 0 到 0.5 的范围内
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_population500', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_nightlight500', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_nightlight500 = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_nightlight500)+
  geom_ribbon(aes(x=nightlight500, y=mean_value, ymin=Low_value, ymax=UP_value),fill="#ED0021",alpha=0.3)+
  geom_line(aes(x=nightlight500, y=mean_value),colour="black",size=0.7)+
  annotate("text",x=50, y=-1.5, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
...
#######################
ggplot(data = query_sample2)+
  geom_histogram(aes(x = nightlight500, y = ..density..), binwidth = 0.1, fill = "#002C5B66", color = "#002c5B66", alpha = 0.5) +
  #geom_density(aes(x=nightlight500),fill="#002C5B66", color="white",show.legend = F, adjust = 1.5)+
  ...
  #labs(x=expression("nightlight500"),y="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.35,b=0,l=0.2,r=0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
        axis.text.x = element_text(color = "black", size = 15)  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
  )->pnightlight5002 # 可视范围限制在 0 到 0.5 的范围内
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_nightlight500', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_settlement500', '.csv', sep="")  # 字符串连接

marginaleffects_UTC500_settlement500 = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_settlement500)+
  geom_ribbon(aes(x=settlement500, y=mean_value, ymin=Low_value, ymax=UP_value),fill="#ED0021",alpha=0.3)+
  geom_line(aes(x=settlement500, y=mean_value),colour="black",size=0.7)+
  annotate("text",x=25, y=-1.2, size=5,color="grey20",hjust=0,label = "italic(P) < 0.005", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0, 100), breaks = seq(0, 100, by =25), 
                     labels = scales::number_format(accuracy = 30.0)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-4.8, 0.0),
                     breaks = seq(-4.8, 0.0, by = 1.2), 
                     labels = scales::number_format(accuracy = 0.1)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
...
#######################
ggplot(data = query_sample2)+
  geom_histogram(aes(x = settlement500, y = ..density..), binwidth = 0.1, fill = "#002C5B66", color = "#002c5B66", alpha = 0.5) +
  #geom_density(aes(x=settlement500),fill="#002C5B66", color="white",show.legend = F, adjust = 1.5)+
  ...
  scale_y_continuous(name = "  ", limits = c(0, 0.26),
                     ...
                     labels = scales::number_format(accuracy = 0.01)) +
  #labs(x=expression("settlement500"),y="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.35,b=0,l=0.2,r=0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
        axis.text.x = element_text(color = "black", size = 15)  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
  )->psettlement5002 # 可视范围限制在 0 到 0.5 的范围内
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_settlement500', '.pdf', sep="")  # 字符串连接
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_impervious500', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_impervious500 = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                                strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_impervious500)+
  geom_ribbon(aes(x=impervious500, y=mean_value, ymin=Low_value, ymax=UP_value),fill="#ED0021",alpha=0.3)+
  geom_line(aes(x=impervious500, y=mean_value),colour="black",size=0.7)+
  annotate("text",x=0.4, y=-1.5, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0.2, 1), breaks = seq(0.2, 1, by =0.20), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-6.2, 0.0),
                     breaks = seq(-6.2, 0.0, by = 1.5), 
                     labels = scales::number_format(accuracy = 1.5)) +
  labs(title = "",y=" ",x="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.3,b=-1.2,l=0.2,r=0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
        axis.text.x = element_text(color = "black", size = 15)  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
  )->pimpervious5001 ## 可视范围限制

print(pimpervious5001)
...
#######################
ggplot(data = query_sample2)+
  geom_histogram(aes(x = impervious500, y = ..density..), binwidth = 0.002, fill = "#002C5B66", color = "#002c5B66", alpha = 0.5) +
  #geom_density(aes(x=impervious500),fill="#002C5B66", color="white",show.legend = F, adjust = 1.5)+
  ...
  scale_y_continuous(name = "  ", limits = c(0, 8),
                     ...
                     labels = scales::number_format(accuracy = 0.1)) +
  #labs(x=expression("impervious500"),y="")+
  theme_classic()+theme0+
  theme(plot.margin = margin(t=-0.35,b=0,l=0.2,r=0.2, unit = "cm"),
        axis.text.y = element_text(color = "black", size = 15),  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
        axis.text.x = element_text(color = "black", size = 15)  # 设置 Y 轴字体颜色为蓝色，字体大小为 12
  )->pimpervious5002 # 可视范围限制在 0 到 0.5 的范围内

print(pimpervious5002)


pimpervious500<-ggarrange(pimpervious5001,pimpervious5002,ncol = 1,align = "v",heights = c(2,1))
#cairo_pdf("figure3.pdf",width = 11,height = 4,family = "Calibri")
print(pimpervious500)


save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_impervious500', '.pdf', sep="")  # 字符串连接

ggsave(save_image_path, plot = pimpervious500, width = 150, height = 150, units = "mm",dpi=200)
...
input_file_name= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_GDP', '.csv', sep="")  # 字符串连接


marginaleffects_UTC500_GDP = read.csv(input_file_name, header=TRUE, sep=',',blank.lines.skip=FALSE, encoding="UTF-8", 
                                      strip.white = FALSE)
...
###################################### 绘制边际效应图和数据分布特征###########################################
ggplot(data = marginaleffects_UTC500_GDP)+
  ...
  annotate("text",x=7, y=-1.6, size=5,color="grey20",hjust=0,label = "italic(P) < 0.001", parse = TRUE)+
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # 添加一条红色虚线
  scale_x_continuous(name = " ", limits = c(0, 20), breaks = seq(0, 20, by =5), 
                     labels = scales::number_format(accuracy = 0.1)) +
  scale_y_continuous(name = "Marginal effect on negative \n sentiment values (95% CI)", limits = c(-6, 0),
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC_GDP', '.pdf', sep="")  # 字符串连接
...
kk <-  cowplot::plot_grid(pimpervious500,nrow = 1, ncol = 1, labels = c("c"),
                          label_size = 18, scale=1,label_x = 0.02, label_y = 1.0, rel_widths = c(1, 1))
...
save_image_path= paste(str0, 'Negative_Sentiment_Rcodes', '/secondary_data_utc500/',  'UTC500_envrionments_Marginal_effect', '.pdf', sep="")  # 字符串连接

ggsave(save_image_path, plot = p8, width = 260, height = 240, units = "mm",dpi=200)
